...
Процедура ВыполнитьРаботу()
	...
	Для Каждого Элемент Из МассивПараметров Цикл
		...
		Если СтрЧислоСтрок(Элемент) < 4 Тогда
			мЛог.Ошибка(СтрШаблон("В файл %1 записаны не все параметры!", Параметры["ПутьКПараметрам"]));	
			Сообщить("В файл param.txt записаны не все параметры");
			ЗавершитьРаботуСкрипта(1);
		КонецЕсли;
		...
		БазаНаДиске = Новый Файл(ПутьКФайлу+"\1Cv8.1CD");
    	Если БазаНаДиске.Существует() Тогда //проверяем доступность базы на текущем компьютере    	
    	...
    	Иначе
        	мЛог.Ошибка(СтрШаблон("По указаному пути %1 не могу найти базу!", Параметры["ПутьКПараметрам"]));        	
    	КонецЕсли
...
Функция СохранениеАрхивов(ПутьКФайлу,ПутьКАрхиву,ИмяБекапа);
	...
	ЗаписьZIP.Открыть(ZipПуть,ZipПароль,ZipКомментарий,,УровеньСжатия);	
